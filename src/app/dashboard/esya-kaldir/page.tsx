...
import { Upload, Camera, Sparkles, X, Loader2, ArrowRight, Plus, Minus, Palette, Shirt, Check, ChevronDown } from "lucide-react";
        ...
        {!resultImage ? (
          ...
          >
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 p-8 shadow-sm transition-colors">
              ...
              {/* ═══════ KİŞİSELLEŞTİRME (ÖN SEÇİM) ═══════ */}
              <div className={`mb-8 rounded-2xl border transition-all duration-300 overflow-hidden ${isCustomizationOpen
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`flex items-center justify-center w-9 h-9 rounded-full transition-colors ${isCustomizationOpen ? "bg-indigo-100 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400" : "bg-gray-100 dark:bg-zinc-800 text-gray-500 dark:text-zinc-400"}`}>
                      <Palette size={16} />
                    </div>
                    <div>
                      <span className={`block text-[14px] font-semibold transition-colors ${isCustomizationOpen ? "text-indigo-900 dark:text-indigo-300" : "text-gray-900 dark:text-zinc-200"}`}>
                        Kişiselleştirme Seçenekleri
                      </span>
                      <span className="block text-[11px] font-light text-gray-500 dark:text-zinc-400 mt-0.5">
                        Renk ve kumaş tipini önceden seçin
                      </span>
                    </div>
                  </div>
                ...
                <AnimatePresence>
                  {isCustomizationOpen && (
                    ...
                    >
                      <div className="px-6 pb-5 pt-1 border-t border-gray-100 dark:border-zinc-800/50">
                        {/* RENK SEÇİMİ */}
                        <div className="mb-6 mt-3">
                          <span className="text-[12px] font-semibold text-gray-600 dark:text-zinc-400 uppercase tracking-wider">Renk Seçimi</span>
                          <div className="flex flex-wrap gap-3 mt-3">
                            {colors.map((c) => (
                              ...
                              >
                                <div
                                  className={`w-9 h-9 rounded-full shadow-md transition-all duration-200 flex items-center justify-center ${selectedColor === c.id
                                ...
                                >
                                  {selectedColor === c.id && (
                                    <Check size={14} className={`${c.id === "antrasit" || c.id === "zumrut" ? "text-white" : "text-gray-700"} drop-shadow-sm`} strokeWidth={3} />
                                  )}
                                </div>
                                <span className={`text-[9px] font-medium transition-colors ${selectedColor === c.id ? "text-indigo-600 dark:text-indigo-400" : "text-gray-400 dark:text-zinc-500"}`}>
                                  {c.name}
                                </span>
                              </button>
                            ))}
                        ...
                        <div>
                          <div className="flex items-center gap-2 mb-3">
                            <Shirt size={14} className="text-teal-500 dark:text-teal-400" />
                            <span className="text-[12px] font-semibold text-gray-600 dark:text-zinc-400 uppercase tracking-wider">Kumaş Tipi</span>
                          </div>
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                            {fabrics.map((f) => (
                              <button
                                ...
                                className={`relative px-3 py-2.5 rounded-xl border text-left transition-all duration-200 ${selectedFabric === f.id
                                    ...
                                    : "border-gray-100 dark:border-zinc-800/80 bg-gray-50/50 dark:bg-zinc-950/50 hover:border-gray-300 dark:hover:border-zinc-700"
                                  }`}
                              >
                                <div className="flex items-center gap-1.5 mb-0.5">
                                  <span className="text-base leading-none">{f.icon}</span>
                                  <span className={`text-[12px] font-semibold transition-colors ${selectedFabric === f.id ? "text-teal-700 dark:text-teal-300" : "text-gray-700 dark:text-zinc-300"}`}>
                                    {f.name}
                                  </span>
                                </div>
                                <span className={`text-[10px] font-light transition-colors ${selectedFabric === f.id ? "text-teal-600/70 dark:text-teal-400/70" : "text-gray-400 dark:text-zinc-500"}`}>
                                  {f.desc}
                                </span>
                                {selectedFabric === f.id && (
                                  <div className="absolute top-1.5 right-1.5">
                                    <Check size={12} className="text-teal-500 dark:text-teal-400" strokeWidth={3} />
                                  </div>
                                )}
                              </button>
                            ))}
                ...
                {isCustomizationOpen && (
                  ...
                  >
                    <div className="px-6 pb-6 pt-2 border-t border-gray-100 dark:border-zinc-800/50">
                      ...
                      {/* ═══════ YENİDEN OLUŞTUR BUTONU ═══════ */}
                      <div className="flex justify-end pt-4 border-t border-gray-100 dark:border-zinc-800/50">
                        <button
                          type="button"
                          onClick={() => handleRecolor(selectedColor, selectedFabric)}
                          disabled={isRecoloring}
                          className="flex items-center gap-2 px-6 py-2.5 rounded-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-medium text-sm transition-colors shadow-sm"
                        >
                          {isRecoloring ? (
                            <>
