...
export async function POST(req: NextRequest) {
  try {
    ...
    // ═══════════════════════════════════════════════════════════
    // AI INPAINTING (TÜM DEĞİŞİMLER İÇİN)
    // Programatik blend maske hatalarını (ahşap ayakların boyanması)
    // engelleyemediği için tamamen semantik anlama sahip AI'a geçtik.
    // ═══════════════════════════════════════════════════════════
    ...
    // Daha mat ve zengin renk tanımları (neon etkisini kırmak için)
    const colorPrompts: Record<string, string> = {
      bej: "rich matte beige cream",
      antrasit: "rich matte dark charcoal grey",
      kiremit: "rich matte terracotta brown",
      zumrut: "rich matte deep forest green",
    };

    const fabricPrompts: Record<string, string> = {
      kadife: "velvet textured",
      keten: "linen textured",
      deri: "leather textured",
      sonil: "chenille textured",
    };

    const colorText = colorPrompts[color] || "";
    const fabricText = fabricPrompts[fabric] || "";
    
    // Yastık sayısını, ayakları ve şekli koruması için kesin komut
    const baseInstruction = "keep the exact same furniture structure, exact same cushions, do not change the wooden legs or environment";
    
    let prompt = "";
    if (colorText && fabricText) {
      prompt = `recolor the fabric to ${colorText} ${fabricText} material, ${baseInstruction}`;
    } else if (colorText) {
